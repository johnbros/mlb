...
with conn.cursor(name='server-cursor') as cur:
    ...
    cur.execute("SELECT game_id, inning_num, inning_half, at_bat_num, pitch_num FROM pitches WHERE inning_num > 1 and at_bat_num > 10000")

    batch = []
    for row in cur:
        game_id, inning_num, inning_half, at_bat_num, pitch_num = row
        batch.append((at_bat_num - 10001, game_id, inning_num, inning_half, at_bat_num, pitch_num))
